...
for dataset_name in ['eval_videomme','eval_mmvu','eval_tempcompass','val_clevrer','eval_mvbench','eval_videoholmes']:
    

    OUTPUT_PATH = f"{MODEL_PATH}/eval_{dataset_name}_{file_name}_greedy_output.json"
    PROMPT_PATH = f"./data/evaluation/{dataset_name}.json"
    ...
    for i in tqdm(range(start_idx, len(messages), BSZ), desc="Processing batches"):
        ...
        # try:
            # Set video/image paths in batch_messages before process_vision_info
        for idx, (msg, sample) in enumerate(zip(batch_messages, data[i:i+BSZ])):
            if msg[0]['content'][0]['type'] == 'video':
                msg[0]['content'][0]['video'] = sample['video']
            elif msg[0]['content'][0]['type'] == 'image':
                msg[0]['content'][0]['image'] = sample['video']
        
        image_inputs, video_inputs, video_kwargs = process_vision_info(batch_messages, return_video_kwargs=True, max_workers=len(batch_messages))
