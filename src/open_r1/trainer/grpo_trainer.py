...
from transformers.utils import is_peft_available, is_flash_attn_2_available
        ...
        if is_deepspeed_zero3_enabled():
            if self.beta != 0:
                if ref_model_path is None:
                    ref_model_path = model_id

                if "Qwen2-VL" in ref_model_path:
                    self.ref_model = Qwen2VLForConditionalGeneration.from_pretrained(ref_model_path, **model_init_kwargs)
                elif "Qwen2.5-VL" in ref_model_path:
                    self.ref_model = Qwen2_5_VLForConditionalGeneration.from_pretrained(ref_model_path, **model_init_kwargs)
                elif "Aria" in ref_model_path:
                    self.ref_model = AriaForConditionalGeneration.from_pretrained(ref_model_path, **model_init_kwargs)
                else:
